...
def place_factory_near_random_ice(key: jax.random.KeyArray, player: int, state):
    ...
    near_ice_positions = jnp.roll(state.board.ice, 2, axis=0) | jnp.roll(state.board.ice, -2, axis=0) | jnp.roll(state.board.ice, 2, axis=1) | jnp.roll(state.board.ice, -2, axis=1)
    ...
    return dict(spawn=spawn, metal=state.teams.init_metal[player], water=state.teams.init_water[player])