!pip install kaggle-environments gym numpy
import gym
import numpy as np
...
def get_action(observation, state, q_table, epsilon):
    if np.random.uniform(0, 1) < epsilon:
        return np.random.choice(observation['actions'])

    actions = observation['actions']
    action_values = q_table[state]

    max_value = np.max(action_values)
    max_actions = [a for a, v in zip(actions, action_values) if v == max_value]
