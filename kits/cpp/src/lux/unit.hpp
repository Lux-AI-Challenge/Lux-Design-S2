...
namespace lux {
    ...
    struct Unit {
        ...
        UnitAction move(Direction direction, int64_t repeat = 0, int64_t n = 1) const;

        UnitAction transfer(Direction direction,
                            Resource  resource,
                            int64_t   amount,
                            int64_t   repeat = 0,
                            int64_t   n      = 1) const;

        UnitAction pickup(Resource resource, int64_t amount, int64_t repeat = 0, int64_t n = 1) const;

        int64_t    digCost(const Observation &obs) const;
        UnitAction dig(int64_t repeat = 0, int64_t n = 1) const;

        int64_t    selfDestructCost(const Observation &obs) const;
        UnitAction selfDestruct(int64_t repeat = 0, int64_t n = 1) const;

        UnitAction recharge(int64_t amount, int64_t repeat = 0, int64_t n = 1) const;
    };
