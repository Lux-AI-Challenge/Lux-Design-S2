...
class SingleUnitObservationWrapper(gym.ObservationWrapper):
    ...
    def observation(self, obs: Dict[str, ObservationStateDict]) -> Dict[str, npt.NDArray]:
        observation = dict()
        ...
        for agent in obs.keys():
            ...
            for k in units.keys():
                ...
                closest_ice_tile = ice_tile_locations[np.argmin(ice_tile_distances)] / self.env.state.env_cfg.map_size
                obs_vec = np.concatenate([unit_vec, factory_vec, closest_ice_tile], axis=-1)
