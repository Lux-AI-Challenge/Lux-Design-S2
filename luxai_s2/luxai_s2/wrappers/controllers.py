...
class Controller:
    ...
    def action_to_lux_action(self, agent: str, obs: Dict[str, ObservationStateDict], act: npt.NDArray):
        """
...
class SimpleDiscreteController(Controller):
    def __init__(self, env_cfg: EnvConfig) -> None:
        ...
        action_space = spaces.Box(0, 1, shape=(env_cfg.map_size, env_cfg.map_size, total_act_dims))
    ...
    def _get_transfer_action(self, id):
        ...
        transfer_dir = id % 5
        resource_type = id // 5
        return np.array([1, transfer_dir, resource_type, self.env_cfg.max_transfer_amount, 0, 1])
    ...
    def action_to_lux_action(self, agent: str, obs: Dict[str, ObservationStateDict], action: npt.NDArray):
        """
        ...
        for unit_id in units.keys():
            ...
            unit_related_action = action_here[:-self.factory_act_dims] # assuming factory action is always the final few dimensions
        ...
        for unit_id in factories.keys():
            ...
            action_here = action[pos[0], pos[1]]
            factory_related_action = action_here[-self.factory_act_dims:]# assuming factory action is always the final few dimensions
