...
import { loadFromObject, loadFromString, loadFromFile } from "./load-replay/utils"
...
export const useStore = create<Store>((set, get) => ({
  ...
  loadReplay: async (action) => {
    switch (action.type) {
      ...
      default: {
        ...
        throw new Error(`invalid action type ${action.type} provided to \`loadReplay\`, or \`loadReplay\` doesn't handle all possible action types`)
      }
    }
  },
  ...
  toggleAutoplay: (to) => set((state) => ({
    autoplay: to ?? !state.autoplay
  })),

  speed: initial.speed,

  updateSpeed: (action) => set((state) => {
    switch (action.type) {
      case 'reset':
